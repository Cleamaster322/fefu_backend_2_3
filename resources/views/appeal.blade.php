    ...
    <body class="antialiased">
        ...
        <form method = "post" action="{{ route('appeal') }}">
            ...
            <div>
                ...
                <input class = "bordered" name = "email" type="text" value = "{{ request()->isMethod('post') ? old('email') : ''}}" maxlength = "100" size = "20"/>
                <p>{{ $errors['email'] ?? '' }}</p>
            </div>

            @if (isset($errors['contacts']))
            <p>Заполнить телефон или мэил</p>
        @endif
            <div>
                <label>Message</label>
                <textarea class = "bordered" name = "message" maxlenght = "50">{{ request()->isMethod('post') ? old('message') : ''}}</textarea>
                <p>{{ $errors['message'] ?? '' }}</p>
            </div>
